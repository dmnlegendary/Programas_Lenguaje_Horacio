...
def construir_matriz_transicion(texto):
    palabras = texto.split()
    if len(palabras) < 6:
        raise ValueError("El texto debe contener al menos 5 palabras.")
    
    frases = [' '.join(palabras[i:i+5]) for i in range(len(palabras) - 4)]
...
def generar_texto_frases(frases_unicas, matriz, longitud_texto):
    ...
    for _ in range(longitud_texto - 5):
        ...
        palabras.extend(siguiente_frase.split()[4:5])  # Solo añadir la última palabra de la frase siguiente
...
def imprimir_matriz(frases_unicas, matriz):
    ...
    with open('matriz5_frec.txt', 'w', encoding='utf-8') as archivo:
